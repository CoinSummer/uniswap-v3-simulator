...
func (pm *Simulator) HandleBlock(block *blockingester.NewBlockMsg) error {
	...
	for _, log := range logs {
		topic0 := log.Topics[0]
		logrus.Infof(topic0.String())
		//poolAddr := log.Address
		//if pool, ok := pm.pools[poolAddr]; !ok {
		//	pm.pools[poolAddr] = NewCorePoolFromConfig(NewPoolConfig())
		//}
		//if topic0 == TOPIC_SWAP {
		//	logrus.Infof("%s %s %s", topic0, pm.SwapID, topic0 == pm.SwapID)
		//}
		if topic0 == pm.InitializeID {
			//swapEvent, err := parseUniv3SwapEvent(&log)
			//if err != nil {
			//	logrus.Infof("skip bad swap event: %s", log.TxHash)
			//}
			logrus.Infof("pool initialize %s", log.TxHash)
		}
	}
	return nil
}
